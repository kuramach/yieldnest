...
function parseMerrill(rows: string[][], headerIdx: number, headers: string[]): ParsedRow[] {
  const result: ParsedRow[] = [];
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    const asset_type: 'stock' | 'etf' | 'bond' = lowerName.includes('etf') ? 'etf'
      ...
      : 'stock';
    result.push({ ticker: ticker.toUpperCase(), name, quantity: qty, cost_basis: costBasis, market_value: marketValue, price, gain_loss: gainLoss, gain_loss_pct: gainLossPct, asset_type });
  }
  return result;
}
