...
import type { BucketHolding, SecurityQuote } from '@/lib/types';
...
export default async function BucketDetailPage({ params }: Props) {
  ...
  if (holdings && holdings.length > 0) {
    const tickers = holdings.map((h: BucketHolding) => h.ticker).join(',');
    try {
      const quoteRes = await fetch(
        `${process.env.NEXT_PUBLIC_SUPABASE_URL ? '' : 'http://localhost:3000'}/api/securities/quote?tickers=${tickers}`,
        { next: { revalidate: 300 } }
      );
      if (quoteRes.ok) {
      ...
      // Quotes unavailable; show holdings without live data
    }
  }
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-slate-400 border-b border-slate-100">
                  ...
                  <th className="text-right px-5 py-2 font-medium">Type</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={h.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-5 py-3 text-right">
                        <span className="text-xs text-slate-400 capitalize">{h.asset_type}</span>
                      </td>
                    </tr>
                  );
                })}
